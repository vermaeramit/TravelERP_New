...
public class RoomTypeRepository : IRoomTypeRepository
{
    ...
    public RoomTypeRepository(DbConnectionFactory factory, ITenantContext tenant)
    {
        _factory = factory;
        _tenant = tenant;
    }
    ...
    public async Task<IEnumerable<RoomType>> GetAllAsync()
    {
        ...
        return await conn.QueryAsync<RoomType>(
            "sp_RoomType_GetAll",
    ...
    public async Task<int> InsertAsync(RoomType roomType)
    {
        using var conn = _factory.CreateMasterConnection();
        var p = new DynamicParameters();
        p.Add("DatabaseName", _tenant.DatabaseName);
        p.Add("Name",         roomType.Name);
        p.Add("CreatedBy",    _tenant.UserId);
        p.Add("NewId", dbType: DbType.Int32, direction: ParameterDirection.Output);
        await conn.ExecuteAsync("sp_RoomType_Insert", p, commandType: CommandType.StoredProcedure);
    ...
    public async Task UpdateAsync(RoomType roomType)
    {
        ...
        await conn.ExecuteAsync(
            "sp_RoomType_Update",
            new { DatabaseName = _tenant.DatabaseName, roomType.Id, roomType.Name, UpdatedBy = _tenant.UserId },
            commandType: CommandType.StoredProcedure);
    }
